$(document).ready(function() {
    window.addEventListener('scroll', () => {
        if (window.pageYOffset > $('.exhibition-page-section').height() - $('.exhibition-card').height()) {
...
function buy_now_form() {
    var buy_now_form = {
        ...
        show_date_picker: function(dates, form, append) {
            ...
            document.querySelector('.buy-now-date-picker').removeAttribute('disabled');
        },
        ...
        triggerClick: function() {
            ...
            $('.site-header .mobile-menu-wrap .menu > li').each(function(index) {
                $(this).on('click', function(e) {
                    ...
                    $(this).siblings('.is-active').removeClass('is-active').find('.sub-menu').slideUp(700);
...
/* Custom */
function open_side_menu(target) {
...
function mobile_footer_nav_init() {
    ...
    if ($(window).width() <= 990) {
        ...
        $('.footer-col-menu .footer-content').each(function() {
            $(this).click(function() {

                $(this).parent().toggleClass('active').find('.footer-nav').slideToggle();

            });
        });

        $('.footer-col-menu a').on('click', function(e) {
            e.stopPropagation();
        });

        // const menu = document.querySelectorAll('.footer-col-menu .entry-title');
    ...
    if ($('.exhibition-page-section').length > 0) {
        ...
        if ($(window).width() > 991) {
            $(window).scroll(function(e) {
                ...
                if (offset + sidebarHeight + sidebarTopOffset > contentOffsetTop + contentHeight) {
                    offset = contentHeight - sidebarHeight;
                }
